...
import {
  Fingerprint, Plus, Wifi, WifiOff, RefreshCw, Loader2,
  MoreVertical, Pencil, Trash2, Activity, Signal, AlertTriangle,
} from 'lucide-react';
...
const EMPTY_FORM: DeviceFormData = {
  name: '', adapter_type: 'mock', serial_number: '',
  branch_public_id: '', ip: '', port: '80',
...
export default function DevicesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const { data } = await apiClient.put(`/devices/${editDevice.public_id}`, buildPayload(form));
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['devices'] });
  ...
  function openEdit(device: Device) {
    setForm({
      ...
      ip: '', port: '80', username: '', password: '', api_key: '',
    });
      ...
      <div className="space-y-6">
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              >
                {syncAllMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}
                Sync All
              </Button>
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            action={<Button onClick={() => { setForm({ ...EMPTY_FORM }); setCreateOpen(true); }}><Plus className="mr-2 h-4 w-4" /> Add Device</Button>}
          />
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {devices.map((d) => (
        ...
        <Dialog open={!!deleteDevice} onOpenChange={(open) => { if (!open) setDeleteDevice(null); }}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to delete <strong>{deleteDevice?.name}</strong>? This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              ...
              <Button
                variant="destructive"
                onClick={() => deleteDevice && deleteMutation.mutate(deleteDevice.public_id)}
...
function DeviceCard({
  device, onPull, onTest, onEdit, onDelete, pulling, testing,
}: {
  device: Device;
  onPull: () => void;
  onTest: () => void;
      ...
      <CardContent className="p-4">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Link href={`/devices/${device.public_id}`} className="font-semibold text-foreground hover:underline">{device.name}</Link>
        ...
        <div className="mt-4 space-y-1.5 text-xs text-muted-foreground">
          {device.serial_number && (
            <p>S/N: <span className="font-mono">{device.serial_number}</span></p>
          )}
          ...
          <p>
            Sync: {device.auto_sync ? `Every ${device.sync_interval_minutes}m` : 'Manual only'}
          </p>
          <p>
            Last sync:{' '}
            {device.last_sync_at
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <Button
            ...
            disabled={testing}
          >
            {testing ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <Signal className="mr-1 h-3 w-3" />}
            Test
          </Button>
          <Button
            variant="outline"
            ...
            disabled={pulling}
          >
            {pulling ? <Loader2 className="mr-1 h-3 w-3 animate-spin" /> : <RefreshCw className="mr-1 h-3 w-3" />}
            Pull
          </Button>
        </div>
      </CardContent>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge variant="outline" className={styles[status] ?? ''}>
      <span className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-full ${dots[status] ?? 'bg-gray-400'}`} />
      {status}
    </Badge>
  );
}
...
function DeviceFormDialog({
  open, onOpenChange, title, form, setForm, branches, isMock, onSubmit, isPending, submitLabel,
}: {
  open: boolean;
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select value={form.adapter_type} onValueChange={(v) => set('adapter_type', v)}>
          ...
          {!isMock && (
            <div className="space-y-4 rounded-lg border p-4">
              <p className="text-xs font-medium text-muted-foreground uppercase">Connection Settings</p>
          ...
          {branches.length > 0 && (
            <div>
              <Label>Branch</Label>
              <Select value={form.branch_public_id} onValueChange={(v) => set('branch_public_id', v)}>
