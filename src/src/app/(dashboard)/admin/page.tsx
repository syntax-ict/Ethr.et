...
import { Building2, TrendingUp, Activity, Users, ArrowRight, ScrollText, XCircle, Pause } from 'lucide-react';
      ...
      <div className="space-y-6">
        ...
        {revLoading ? (
          ...
          <>
            ...
            {(revenue.suspended_tenants > 0 || revenue.cancelled_tenants > 0) && (
              <div className="grid gap-4 sm:grid-cols-2">
                <MetricCard icon={Pause} title="Suspended" value={String(revenue.suspended_tenants ?? 0)} color="amber" />
                <MetricCard icon={XCircle} title="Cancelled" value={String(revenue.cancelled_tenants ?? 0)} color="red" />
              </div>
            )}
          </>
        ) : null}
            ...
            {!audit ? (
              ...
              <div className="space-y-2">
                {audit.data.slice(0, 10).map((log) => (
                  <div key={log.id} className="flex items-center justify-between gap-3 rounded-lg border p-2">
