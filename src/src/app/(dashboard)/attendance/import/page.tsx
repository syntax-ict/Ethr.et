...
import {
  ...
  Loader2, ArrowLeft, Download, Trash2,
} from 'lucide-react';
...
export default function AttendanceImportPage() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const { data } = await apiClient.post('/attendance/import/preview', formData, {
          ...
          <CardContent className="p-8">
            ...
            >
              {uploading ? (
                <>
                  ...
                  <p className="mt-4 text-sm font-medium">Parsing {fileName}…</p>
                </>
              ) : (
                <>
            ...
            <div className="mt-6 rounded-lg bg-muted/50 p-4">
              ...
              <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
                <li>- <strong>employee_code</strong>: Employee ID/code in your system (required)</li>
                <li>- <strong>date</strong>: YYYY-MM-DD format (required)</li>
                <li>- <strong>check_in_time</strong>: HH:MM 24-hour format (required)</li>
                <li>- <strong>check_out_time</strong>: HH:MM 24-hour format (optional)</li>
              </ul>
            </div>
          </CardContent>
        </Card>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b bg-muted/50">
                      <th className="px-4 py-3 text-left text-xs font-medium uppercase text-muted-foreground w-12">Line</th>
                      <th className="px-4 py-3 text-left text-xs font-medium uppercase text-muted-foreground w-12">Status</th>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium uppercase text-muted-foreground">Issues</th>
                    </tr>
                  </thead>
                  <tbody>
                    {preview.rows.map((row, i) => (
                      ...
                      >
                        <td className="px-4 py-2 text-xs text-muted-foreground">{row.line}</td>
                        ...
                        <td className="px-4 py-2 text-sm font-mono">{row.employee_code}</td>
                        <td className="px-4 py-2 text-sm">{row.date}</td>
                        <td className="px-4 py-2 text-sm">{row.check_in_time}</td>
                        <td className="px-4 py-2 text-sm text-muted-foreground">{row.check_out_time || '—'}</td>
                        <td className="px-4 py-2">
                          {row.errors.length > 0 && (
                            <div className="space-y-0.5">
                              {row.errors.map((err, j) => (
                                <p key={j} className="text-xs text-red-600 dark:text-red-400">{err}</p>
                              ))}
                            </div>
                          )}
                        </td>
      ...
      {step === 'done' && importResult && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <h2 className="mt-4 text-lg font-semibold text-foreground">Import Complete</h2>
            <div className="mt-4 flex gap-4">
              <Badge variant="outline" className="px-3 py-1.5 text-sm bg-green-50 dark:bg-green-950/30">
                {importResult.created} created
              </Badge>
              {importResult.skipped > 0 && (
                <Badge variant="outline" className="px-3 py-1.5 text-sm bg-amber-50 dark:bg-amber-950/30">
