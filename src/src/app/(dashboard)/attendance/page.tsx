...
export default function AttendancePage() {
  ...
  function handleCheckOut() {
    checkOut.mutate({ idempotency_key: crypto.randomUUID() }, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 border-b pb-3">
        <SubNav href="/attendance/scan" icon={QrCode}>Scan QR</SubNav>
        ...
        <>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b bg-muted/50">
                      ...
                      <th className="hidden px-4 py-3 text-left text-xs font-medium uppercase text-muted-foreground sm:table-cell">Source</th>
          ...
          {data?.meta && data.meta.last_page > 1 && (
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">Showing {data.meta.from}–{data.meta.to} of {data.meta.total}</p>
...
function SubNav({ href, icon: Icon, children }: { href: string; icon: React.ComponentType<{ className?: string }>; children: React.ReactNode }) {
  return (
    <Button asChild variant="ghost" size="sm" className="h-8">
      <Link href={href}><Icon className="mr-2 h-3 w-3" /> {children}</Link>
    </Button>
  );
}
...
function ManualEntryDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { data: employees } = useQuery({
    ...
    queryFn: async () => (await apiClient.get('/employees', { params: { per_page: 100 } })).data,
    enabled: open,
  });
      ...
      <DialogContent>
        ...
        <form onSubmit={(e) => { e.preventDefault(); submit.mutate(); }} className="space-y-3">
          <div>
            ...
            <Select value={form.employee_public_id} onValueChange={(v) => setForm((p) => ({ ...p, employee_public_id: v }))}>
              <SelectTrigger className="mt-1"><SelectValue placeholder="Select employee" /></SelectTrigger>
              <SelectContent>
                {employees?.data?.map((e: { public_id: string; name: string; employee_code: string }) => (
                  <SelectItem key={e.public_id} value={e.public_id}>
                    {e.name} {e.employee_code && <span className="text-muted-foreground">({e.employee_code})</span>}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Input type="time" value={form.check_in} onChange={(e) => setForm((p) => ({ ...p, check_in: e.target.value }))} required className="mt-1" />
            </div>
            <div>
              <Label>Check Out</Label>
              <Input type="time" value={form.check_out} onChange={(e) => setForm((p) => ({ ...p, check_out: e.target.value }))} className="mt-1" />
            </div>
          </div>
          <div>
            <Label>Reason *</Label>
            <Textarea value={form.reason} onChange={(e) => setForm((p) => ({ ...p, reason: e.target.value }))} placeholder="Why is this manual entry needed?" required rows={2} className="mt-1" />
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={onClose}>Cancel</Button>
            <Button type="submit" disabled={submit.isPending || !form.employee_public_id}>
