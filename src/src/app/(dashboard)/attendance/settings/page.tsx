...
const ALL_METHODS = [
  { key: 'biometric', label: 'Biometric Device', description: 'Fingerprint, face recognition devices' },
  { key: 'mobile', label: 'Mobile Check-in', description: 'GPS + selfie from employee phone' },
  { key: 'qr', label: 'QR Code Scan', description: 'Scan QR at branch entrance' },
  { key: 'kiosk', label: 'Kiosk', description: 'Shared device with employee code' },
  { key: 'web', label: 'Web Portal', description: 'Check-in from dashboard' },
  { key: 'manual', label: 'Manual Entry', description: 'HR/Admin manual recording' },
  { key: 'csv', label: 'CSV Import', description: 'Bulk import from spreadsheet' },
];
...
export default function AttendanceSettingsPage() {
  ...
  function setField<K extends keyof AttendanceSettings>(key: K, value: AttendanceSettings[K]) {
    if (!form) return;
    ...
    <RoleGate minRole="hr_admin">
      <div className="space-y-6">
        <PageHeader
          ...
          actions={
            <Button onClick={() => form && save.mutate(form)} disabled={!form || save.isPending}>
        ...
        {isLoading || !form ? (
          ...
          <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {ALL_METHODS.map((m) => {
                    ...
                    return (
                      ...
                      >
                        <Switch checked={enabled} onCheckedChange={() => toggleMethod(m.key)} />
                        <div>
            ...
            {/* Mobile Settings */}
            <Card>
              <CardHeader><CardTitle className="text-base">Mobile Check-in</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">Require geofence</p>
                    <p className="text-xs text-muted-foreground">Reject check-ins outside branch area</p>
                  </div>
                  <Switch
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Selfie required for check-in</p>
                  </div>
                  <Switch
                    checked={form.mobile_photo_required}
                    onCheckedChange={(v) => setField('mobile_photo_required', v)}
                  />
                </div>
                <div>
                  <Label className="text-sm">GPS accuracy threshold (meters)</Label>
                  <Input
                    type="number"
                    value={form.mobile_accuracy_threshold_meters}
                    onChange={(e) => setField('mobile_accuracy_threshold_meters', parseInt(e.target.value) || 100)}
                    min={10}
                    max={5000}
                    className="mt-1 w-32"
                  />
                  <p className="mt-1 text-xs text-muted-foreground">Reject if GPS accuracy exceeds this</p>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">Offline sync</p>
                    <p className="text-xs text-muted-foreground">Allow check-in when offline</p>
                  </div>
                  <Switch
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setField('qr_expiry_minutes', parseInt(e.target.value) || 30)}
                    min={5}
                    max={480}
                    className="mt-1 w-32"
                  />
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Auto-regenerate when QR expires</p>
                  </div>
                  <Switch
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setField('qr_single_use_limit', parseInt(e.target.value) || 0)}
                    min={0}
                    max={1000}
                    className="mt-1 w-32"
                  />
                  <p className="mt-1 text-xs text-muted-foreground">0 = unlimited scans per QR</p>
                </div>
              </CardContent>
            </Card>
            ...
            {/* Kiosk Settings */}
            <Card>
              <CardHeader><CardTitle className="text-base">Kiosk</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">Require employee PIN</p>
                    <p className="text-xs text-muted-foreground">Employees must enter PIN after code</p>
                  </div>
                  <Switch
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setField('kiosk_auto_reset_seconds', parseInt(e.target.value) || 4)}
                    min={2}
                    max={30}
                    className="mt-1 w-32"
                  />
                  <p className="mt-1 text-xs text-muted-foreground">Reset kiosk after success/error</p>
                </div>
              </CardContent>
            </Card>
          </div>
