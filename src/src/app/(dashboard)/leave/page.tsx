...
import { CalendarDays, Plus, Loader2, Check, X, List, LayoutGrid, ChevronLeft, ChevronRight } from 'lucide-react';
...
import { useLeaveBalance, useMyLeaveRequests, useLeaveTypes, useSubmitLeave, useTeamLeaveRequests, useApproveLeave, useRejectLeave, type LeaveBalance, type LeaveRequest } from '@/features/leave/api';
...
function TeamLeaveTab() {
  ...
  const { data, isLoading } = useTeamLeaveRequests({ page, per_page: 100 } as { page: number });
  ...
  return (
    <div className="space-y-4">
      ...
      {view === 'calendar' ? (
        <TeamLeaveCalendar requests={requests as (LeaveRequest & { employee_name?: string })[]} />
      ) : (
        <Card>
          <CardHeader><CardTitle className="text-base">Team Leave Requests</CardTitle></CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              <div className="space-y-3 p-4">{Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div>
            ) : requests.length === 0 ? (
...
function TeamLeaveCalendar({ requests }: { requests: (LeaveRequest & { employee_name?: string })[] }) {
    ...
    <Card>
      ...
      <CardContent className="p-0 pb-4 px-4">
        <div className="grid grid-cols-7 gap-px rounded-lg border bg-muted/50 overflow-hidden">
          ...
          {cells.map((day, i) => {
            if (day === null) {
              return <div key={`empty-${i}`} className="min-h-[4.5rem] bg-background" />;
            }
        ...
        <div className="mt-3 flex flex-wrap gap-3 text-xs">
          <span className="flex items-center gap-1">
            <span className="inline-block h-2.5 w-2.5 rounded-sm bg-green-200 dark:bg-green-900" /> Approved
          </span>
          <span className="flex items-center gap-1">
            <span className="inline-block h-2.5 w-2.5 rounded-sm bg-amber-200 dark:bg-amber-900" /> Pending
          </span>
        </div>
      </CardContent>
    </Card>
