...
import {
  BarChart3, Download, Loader2, Users, Clock, CalendarDays, Wallet,
  Database, Columns3, ListFilter, Group, ArrowDownUp, Eye, Save,
  Bookmark, CalendarClock, Trash2, Mail, Plus, X, AlertCircle,
...
import {
  useReportSources, useGenerateReport, useSavedReports, useSaveReport,
  useDeleteSavedReport, useScheduledReports, useScheduleReport, useDeleteScheduledReport,
  type ReportConfig, type ReportSources, type SavedReport,
...
const prebuilt = [
  ...
  { key: 'attendance', icon: Clock, title: 'Attendance Summary', description: 'Attendance records', color: 'text-green-600' },
  { key: 'leave', icon: CalendarDays, title: 'Leave Balance Report', description: 'Leave balances for all', color: 'text-purple-600' },
  ...
  return (
    <RoleGate minRole="hr_admin">
      <div className="space-y-6">
        <PageHeader title="Reports" description="Build custom reports, save templates, and schedule recurring delivery" />
        ...
        <Tabs defaultValue="builder">
          ...
          <TabsContent value="builder" className="mt-4"><BuilderTab /></TabsContent>
          <TabsContent value="quick" className="mt-4"><QuickTab /></TabsContent>
...
function BuilderTab() {
  ...
  function toggleColumn(field: string) {
    ...
    const next = current ? selectedColumns.filter((f) => f !== field) : [...selectedColumns, field];
  ...
  function buildPayload(): ReportConfig {
    ...
    if (validFilters.length > 0) {
      payload.filters = Object.fromEntries(validFilters.map((f) => [f.field, f.value]));
    }
    return payload;
  }
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="mt-2 text-xs text-muted-foreground">{availableFields.length} available fields</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-base">
              ...
              <Badge variant="outline" className="ml-1 text-[10px]">{selectedColumns.length || 'All'}</Badge>
            </CardTitle>
            <div className="flex gap-1">
              <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={selectAllColumns}>All</Button>
              <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={clearColumns}>Clear</Button>
            </div>
          </CardHeader>
          <CardContent>
            <div className="max-h-56 space-y-1 overflow-y-auto pr-1">
              {availableFields.map((field) => (
                <label key={field} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-muted/50">
                  <input
                    type="checkbox"
                    checked={selectedColumns.length === 0 || selectedColumns.includes(field)}
        ...
        <Card>
          <CardHeader className="pb-3 flex flex-row items-center justify-between">
            ...
            <Button size="sm" variant="ghost" className="h-6 px-2" onClick={addFilter}>
            ...
            {filters.length === 0 ? (
              ...
              filters.map((f, i) => (
                <div key={i} className="flex gap-1">
                  ...
                  />
                  <Button size="sm" variant="ghost" className="h-8 w-8 p-0" onClick={() => removeFilter(i)}>
                    <X className="h-3 w-3" />
                  </Button>
                </div>
              ))
        ...
        <Card>
          ...
          <CardContent>
            <Select
              ...
              onValueChange={(v) => setConfig((p) => ({ ...p, group_by: v === '__none__' ? undefined : v }))}
            >
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <Select
              ...
              onValueChange={(v) => setConfig((p) => ({ ...p, sort_by: v === '__none__' ? undefined : v }))}
            >
            ...
            {config.sort_by && (
              <Select
                ...
                onValueChange={(v) => setConfig((p) => ({ ...p, sort_dir: v as 'asc' | 'desc' }))}
              >
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
        ...
        <div className="flex flex-col gap-2">
          <Button onClick={handlePreview} disabled={generate.isPending}>
            {generate.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Eye className="mr-2 h-4 w-4" />}
            Run Preview
          </Button>
          {generate.data && (
            <>
        ...
        {!generate.data && !generate.isPending && (
          <Card>
            <CardContent className="p-12 text-center text-muted-foreground">
              ...
              <p className="mt-4 text-sm">Configure your report on the left, then click <span className="font-semibold">Run Preview</span></p>
            </CardContent>
          </Card>
        )}
...
function PreviewResult({ result }: { result: { source: string; total: number; data: Array<Record<string, unknown>>; summary: { grouped_by?: string; groups?: Record<string, number> } } }) {
  const headers = useMemo(() => (result.data.length > 0 ? Object.keys(result.data[0]) : []), [result.data]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-muted-foreground">
            <span className="capitalize">{result.source}</span> · {result.total} records
          </p>
        </div>
      </div>
      ...
      {groups && Object.keys(groups).length > 0 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Grouped by {result.summary.grouped_by}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {result.data.length === 0 ? (
        <EmptyState icon={FileSpreadsheet} title="No data" description="The query returned no rows" />
      ) : (
        <Card>
          <CardContent className="p-0">
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-muted">
                  <tr className="border-b">
                    {headers.map((col) => (
                      <th key={col} className="px-3 py-2 text-left font-medium text-muted-foreground capitalize whitespace-nowrap">
            ...
            {result.data.length > 200 && (
              <div className="border-t bg-muted/30 px-4 py-2 text-xs text-muted-foreground">
                Showing first 200 of {result.data.length} rows. Download CSV for full data.
              </div>
            )}
          </CardContent>
        </Card>
...
function SaveReportDialog({ open, onClose, config }: { open: boolean; onClose: () => void; config: ReportConfig }) {
  const [name, setName] = useState('');
  const save = useSaveReport();

  function handleSave() {
    if (!name.trim()) return;
    save.mutate({ name: name.trim(), config }, {
      onSuccess: () => {
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-lg border bg-muted/30 p-3">
            ...
            <div className="mt-2 space-y-1 text-xs">
              <p><span className="text-muted-foreground">Source:</span> <span className="font-mono">{config.source}</span></p>
              {config.columns && <p><span className="text-muted-foreground">Columns:</span> {config.columns.length}</p>}
              {config.filters && <p><span className="text-muted-foreground">Filters:</span> {Object.keys(config.filters).length}</p>}
              {config.group_by && <p><span className="text-muted-foreground">Group by:</span> <span className="font-mono">{config.group_by}</span></p>}
              {config.sort_by && <p><span className="text-muted-foreground">Sort:</span> <span className="font-mono">{config.sort_by} {config.sort_dir ?? 'asc'}</span></p>}
            </div>
          </div>
        </div>
        <DialogFooter>
...
function QuickTab() {
  ...
  function handleGenerate(source: string) {
    generate.mutate({ source }, {
      onSuccess: () => toast.success(`Generated ${generate.data?.total ?? 0} records`),
      onError: () => toast.error('Failed to generate report'),
    });
  }
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        {prebuilt.map((r) => {
          const Icon = r.icon;
          const isLoading = generate.isPending && generate.variables?.source === r.key;
          return (
            <Card key={r.key} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                ...
                <Button size="sm" variant="outline" onClick={() => handleGenerate(r.key)} disabled={isLoading}>
      ...
      {generate.data && (
        <>
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">{generate.data.total} records · <span className="capitalize">{generate.data.source}</span></p>
            <Button size="sm" onClick={downloadCsv}><Download className="mr-2 h-3 w-3" /> Download CSV</Button>
          </div>
...
function SavedTab() {
  ...
  function handleRun(r: SavedReport) {
    runReport.mutate(r.config, {
      onSuccess: () => { setResultFor(r.public_id); toast.success(`Generated ${runReport.data?.total ?? 0} records`); },
  ...
  return (
    <div className="space-y-4">
      {isLoading ? (
        ...
        <EmptyState icon={Bookmark} title="No saved templates" description="Build a report and save it as a template to reuse it later" />
      ) : (
              ...
              <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                    <Badge variant="outline" className="capitalize">{r.config.source}</Badge>
                    {r.config.columns && <span>· {r.config.columns.length} columns</span>}
                    {r.config.filters && <span>· {Object.keys(r.config.filters).length} filters</span>}
                    {r.config.group_by && <span>· grouped by {r.config.group_by}</span>}
                    <span>· saved {new Date(r.created_at).toLocaleDateString()}</span>
                  </div>
                </div>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" onClick={() => handleRun(r)} disabled={runReport.isPending}>
...
function ScheduleDialog({ report, onClose }: { report: SavedReport; onClose: () => void }) {
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            <span className="font-medium">{report.name}</span> will be sent automatically.
          </DialogDescription>
        </DialogHeader>
          ...
          <div>
            ...
            <Select value={frequency} onValueChange={(v) => setFrequency(v as 'daily' | 'weekly' | 'monthly')}>
              ...
              <SelectContent>
                ...
                <SelectItem value="monthly">Monthly — 1st of month 6:00 AM</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div>
            ...
            {recipients.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {recipients.map((r) => (
                  <Badge key={r} variant="outline" className="flex items-center gap-1 text-xs">
                    <Mail className="h-3 w-3" /> {r}
                    <button onClick={() => setRecipients((p) => p.filter((x) => x !== r))} className="ml-1 hover:text-destructive">
                      <X className="h-3 w-3" />
                    </button>
                  </Badge>
                ))}
        ...
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button onClick={handleSchedule} disabled={schedule.isPending || recipients.length === 0}>
              ...
              <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-foreground truncate">{s.report_name}</p>
                    <Badge variant="outline" className="capitalize text-[10px]">{s.frequency}</Badge>
                  </div>
                  <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1">
                      <CalendarClock className="h-3 w-3" /> next: {new Date(s.next_run_at).toLocaleString()}
                    </span>
                    {s.last_run_at && (
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {s.recipients.map((r) => <Badge key={r} variant="outline" className="text-[10px]">{r}</Badge>)}
                  </div>
                </div>
                <Button size="sm" variant="ghost" onClick={() => handleDelete(s.public_id, s.report_name)}>
