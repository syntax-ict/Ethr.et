    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <Card>
        <CardContent className="p-6 sm:p-8">
          ...
          {currentStep === 2 && (
            <OrganizationStep me={me} onNext={next} onBack={back} />
          )}
          {currentStep === 3 && (
            <BrandingStep onNext={next} onBack={back} onSkip={skip} />
          )}
          {currentStep === 4 && (
            <TemplateStep onNext={next} onBack={back} onSkip={skip} />
          )}
          {currentStep === 5 && (
            <BranchStep onNext={next} onBack={back} onSkip={skip} />
          )}
          {currentStep === 6 && (
            <InviteStep me={me} onNext={next} onBack={back} onSkip={skip} />
          )}
...
function WelcomeStep({ me, onNext }: { me?: MeResponse; onNext: () => void }) {
  ...
  return (
    <div className="space-y-6 text-center">
      ...
      <div className="mx-auto max-w-md space-y-3 rounded-xl border bg-muted/30 p-5 text-left">
        ...
        {adminEmail && (
          <div>
            ...
            <code className="mt-1 block rounded bg-background px-3 py-2 text-sm font-mono">
              {adminEmail}
            </code>
          </div>
        )}
      ...
      <p className="text-sm text-muted-foreground">
        Let&apos;s set up your organization in{" "}
        <span className="font-medium text-foreground">5 quick steps</span>.
        <br />
...
function TemplateStep({
  ...
  async function apply() {
    ...
    try {
      ...
      toast.success(
        "Template applied — departments, positions, shifts, and leave types created",
      );
      onNext();
    } catch {
    ...
    <div className="space-y-6">
      ...
      {templates.length === 0 ? (
        <p className="py-6 text-center text-sm text-muted-foreground">
          No templates available — you can skip and add things manually later.
        </p>
      ) : (
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {t.template_data?.departments && (
                    <Badge variant="outline" className="text-[10px]">
                      {t.template_data.departments.length} depts
                    </Badge>
                  )}
                  {t.template_data?.positions && (
                    <Badge variant="outline" className="text-[10px]">
                      {t.template_data.positions.length} positions
                    </Badge>
                  )}
                  {t.template_data?.leave_types && (
                    <Badge variant="outline" className="text-[10px]">
                      {t.template_data.leave_types.length} leave types
                    </Badge>
                  )}
                </div>
              </button>
...
function BranchStep({
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await apiClient.post("/organization/branches", {
        name,
        city,
        code,
        is_active: true,
...
function InviteStep({
  ...
  const parsed = useMemo(
    () =>
      emails
        .split(/[\s,;\n]+/)
        .map((e) => e.trim())
        .filter((e) => e.includes("@") && e !== me?.user?.email),
  ...
  function done() {
    if (parsed.length > 0) {
      toast.success(
        `${parsed.length} invitation${parsed.length > 1 ? "s" : ""} noted (add them properly from Employees later)`,
      );
    }
    onNext();
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground">
        💡 <span className="font-medium text-foreground">Tip:</span> Want to
        bulk-add dozens of employees? Skip this and use{" "}
        <span className="font-mono">Employees → Import CSV</span> from the
        dashboard.
      </div>
