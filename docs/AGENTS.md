...
`AppServiceProvider::boot()` (error log with job/queue/attempts/exception, plus
Sentry when bound), because a retry exhausting itself looks exactly like nothing
happening.

**There is no `failed_syncs` table** — specified here but never built, and the
two rows above claimed a recovery mechanism that does not exist (audit F-5,
2026-08-23). Offline attendance sync is not a queue: `OfflineSyncController`
processes the batch synchronously and returns per-record
`created`/`duplicate`/`error` results, so failures are reported in the response
that asked for them. Records are idempotency-keyed, so the client's IndexedDB
queue retrying is the recovery path, and replay returns `duplicate`.
