...
| 18 | **Nothing transitions an invoice from `draft` to `sent`, so dunning never starts** — no reminders, no past-due, no suspensions | §15d **[verified]** | **High — owner decision** |
...
**Open — nothing ever sends an invoice, so dunning never starts at all.**
...
**Not fixed here, deliberately.** Whether a generated invoice counts as "sent" — or whether there is meant to be a separate send step that emails it first — is a billing-process decision, not a code-correctness one. Guessing would either start dunning customers who were never actually invoiced, or leave a send step half-built. `tests/Feature/Billing/OverdueInvoiceEscalationTest.php` pins the current behaviour as a `todo` so it fails the day someone changes it.
