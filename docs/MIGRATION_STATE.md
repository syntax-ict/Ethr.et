...
| --- | --- | --- | --- | --- | --- |
| **0a** | **Is the VPS still serving?** — *no Plesk needed, but **not answerable from the agent environment*** | n/a — `curl -sI http://91.99.81.71/`, check 80/443. **Attempted 2026-09-24 from the agent sandbox: port 80 returned `403` in 0.002s and port 443 returned `CONNECT tunnel failed, response 403`. Neither is a host result** — an allowed host (`github.com`) takes 0.269s from the same shell, and the proxy logs `connect_rejected … policy denial`. Two milliseconds is the egress proxy refusing, not Hetzner answering. **Run it from an unproxied machine.** | Whether anything answers | No | **B-6.** Decides whether a rollback target exists at all. **Highest priority in this table** |
...
| ~~**5**~~ | ~~**Identify the unexplained object**~~ **ANSWERED 2026-09-24 — it is the live document root.** The served directory is `ethr.et/`, owner-confirmed; the canary uploaded in action 7 landed inside it, which is what settled it | Websites & Domains | — | **NO. Do not delete it** — that would take the site down and break ACME renewal. B-3 is closed as *identified*, not removed | **B-3 closed.** **Action 8 is unblocked** |
| **6** | **Capability probe, Route C** | File Manager → upload to `<DOCROOT>/<random>.php` — **`httpdocs/`**, measured over HTTP 2026-09-24 (a same-day reading of `ethr.et/` was wrong; no move ever happened), **then set `ETHR_PROBE_WEB_TOKEN` in that copy to a second random value** — as shipped every web request returns 403 | The full output. Open it as `?token=<that value>` and pass **no other query parameter**; **delete the file in the same sitting** | Upload, edit the token, then delete | **G0-E**, **G0-H**, storage rows, **G0-J** CPU half. Read the truncation table in `deployment/GATE-0-RESULT.md` Step 1 first |
| **7** | **Canary — five gates, SIX fetches** | File Manager → `<DOCROOT>/ethr-canary/` — **`httpdocs/ethr-canary/`**, measured over HTTP 2026-09-24. **The existing upload landed in `ethr.et/`, which is not served, so it must be re-uploaded** — fetching it returns 404, indistinguishable from a failing gate | Output of all six fetches — **G0-B.5 needs two, `shadow.txt` AND `shadow.js`** — each `curl` with `--resolve www.ethr.et:443:213.55.96.154`; plus the `favicon.ico` header comparison for G0-B.2's static-asset scope | Upload then delete the directory | **G0-B.1–B.5**. Upload **five** files from `main` — `.htaccess`, `canary.php`, `secret.txt.probe`, `shadow.txt`, **`shadow.js`**. `README.md` stays in the repository; it explains the baits and there is no reason to publish that. **`shadow.js` was added 2026-09-18 and is the bait that counts**: with a static block that excludes `.txt`, `shadow.txt` alone reports "the rewrite won" while every asset the deployment ships is being shadowed — a false PASS on G0-B.5 that also hides G0-B.2. **The two can legitimately disagree; record both.** ~~Take the four files from branch `claude/gate0-procedure-corrections`~~ *(corrected 2026-09-18: `shadow.txt` is on `main` since `7fb91cc`; that branch is merged and this row would have sent you to a stale copy).* Miss `shadow.txt` and you run five checks, not six. **Miss `shadow.js` and you run six checks and get the wrong answer** — which is worse |
