...
| **Node.js runtime (Next SSR)** | **Yes, as currently built** | `output: "standalone"` | ~~**UNKNOWN** — Plesk has a Node.js extension; availability unconfirmed~~ **MEASURED 2026-09-22 — PRESENT and startable.** The panel offers a Node.js **application** (version 22.23.2, startup file, application mode, application URL), not a build-only toolchain; Node.js is **not currently enabled**. This settles availability, **not** routing — G0-A remains `NOT VERIFIED`, and G0-D remains FAIL, so this does not unblock Branch A. See `deployment/GATE-0-RESULT.md` → G0-G. | Second blocking gate. See §E. |
