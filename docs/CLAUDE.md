...
| # | Convention | Detail |
|---|---|---|
| 1 | Tenant isolation | `tenant_id` on all scoped tables. `BelongsToTenant` trait with a **fail-closed** global scope (no tenant context applies `whereRaw('0 = 1')`, so absence yields no rows rather than all rows). `TenantIsolationTest` validates every model — run by `./scripts/gates.sh` and by CI on every push. Every `withoutGlobalScope` bypass must re-apply a tenant predicate; 161 sites across 55 files do, and `TenantScopeBypassInventoryTest` fails when that count moves — though a count cannot audit the bypasses that already exist. |
...
**Tenant Isolation (run by `./scripts/gates.sh`, and by CI on every push):**
