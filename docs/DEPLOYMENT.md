...
|---|---|---|
| **Income tax brackets** | ⚠️ **Was stale — updated to Proclamation 1395/2025 on 2026-08-29** | **979/2016 was superseded on 7 July 2025 and this deployment had not been updated**, so every payslip since then over-deducted tax — worst at the bottom, where a 2,000 ETB earner was charged 157.50 ETB/month (~8% of gross) on income that is now exempt. The platform ladder is now the six-band 1395/2025 schedule, effective-dated so pre-amendment periods still reprocess at the old rates. **Two things remain for you:** (1) have a tax adviser or ERCA confirm the schedule below before the first live payroll — this was sourced from legal-firm and payroll-provider summaries, not the Gazette text; (2) **tenants with their own bracket overrides were NOT migrated** and still use 979/2016 — update them via `PUT /api/v1/payroll/tax-brackets`. If any payroll already ran on the old ladder, over-deducted tax must be reconciled with the employees affected. |
| **Employment income tax schedule in force** | Proclamation 1395/2025, effective 7 July 2025 | 0–2,000 → 0% · 2,001–4,000 → 15% · 4,001–7,000 → 20% · 7,001–10,000 → 25% · 10,001–14,000 → 30% · over 14,000 → 35% (monthly ETB). Pension is unchanged at 7% employee / 11% employer and does **not** reduce taxable income. |
...
| **Plan tier feature gating** | ~~Partial~~ **Implemented 2026-08-29** | `Plan.features` is now enforced by `PlanFeatureService` + the `RequiresPlanFeature` middleware, alongside `PlanLimitService`'s existing count caps. Gated: payroll processing/approve/void/reprocess, report generate/export, saved and scheduled report definitions (`custom_reports`), webhook create/update, API-key issuance, and audit-log reads. **Fail-open by design** — trial tenants, tenants with no subscription, and plans with a NULL feature list are never gated, because a billing control wrong in the closed direction locks paying customers out. Reads stay open so a downgraded tenant keeps sight of payroll already run and can still delete a live webhook; the audit log is the deliberate exception, since there the read *is* the capability. |
